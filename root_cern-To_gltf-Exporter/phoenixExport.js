...
import * as JSROOT from 'https://root.cern/js/7.6.1/modules/main.mjs';
import * as JSROOTgeo from 'https://root.cern/js/7.6.1/modules/geom/geobase.mjs';
import * as JSROOTgeoBuild from 'https://root.cern/js/7.6.1/modules/geom/TGeoPainter.mjs';
...
async function internal_convert_geometry(obj, filename, max_level, subparts, hide_children, body, nFaces) {
    ...
    for (const [name, entry] of Object.entries(subparts)) {
        ...
        body.innerHTML += "  CLEANED GEOMETRY " + "</br>"
        // dump to gltf, using one scene per subpart
        // set nb of degrees per face for circles approximation to nFaces
        JSROOTgeo.geoCfg('GradPerSegm', 360/nFaces);
        const paths = entry[0];
        body.innerHTML += "  PATHS " + paths + "</br>"
        const visibility = entry[1];
        body.innerHTML += "  VISIBILITY " + visibility + "</br>"
        ...
        body.innerHTML += "  SET INVISIBLE " + "</br>"
        // convert to gltf
        var scene = new THREE.Scene();
        scene.name = name;
        body.innerHTML += "  SCENE NAME " + scene.name + "</br>"
        var children = JSROOTgeoBuild.build(obj, {dflt_colors: true, vislevel:10, numfaces: 10000000, numnodes: 500000});
        body.innerHTML += "  GOT CHILDREN FOR " + scene.name + "</br>"
        ...
        if (typeof visibility == "boolean") {
            ...
            body.innerHTML += " visible" + "</br>"
        } else {
            scene.userData = {"visible" : true, "opacity" : visibility};
            body.innerHTML += " visible + opacity" + "</br>"
        }
        scenes.push(scene);
    }
